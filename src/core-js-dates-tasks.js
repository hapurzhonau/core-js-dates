 ...
 * 2, 2024 => 29
 */
function getCountDaysInMonth(month, year, day = 0) {
  const daysInMonth = new Date(year, month, day);
...
function getCountDaysOnPeriod(dateStart, dateEnd) {
  ...
  const daysOnPeriod = end - start;
  return daysOnPeriod / 86400000 + 1;
}
...
function isDateInPeriod(date, period) {
  ...
  return dateIn >= startDay && dateIn <= endDay;
}
...
function formatDate(date) {
  ...
  const M = format.getUTCMonth();
  const YYYY = format.getUTCFullYear();
  const D = format.getUTCDate();
  let hh = String(format.getUTCHours()).padStart(2, 0);
  const mm = String(format.getUTCMinutes()).padStart(2, 0);
  const ss = String(format.getUTCSeconds()).padStart(2, 0);
  const a = hh >= 12 ? 'PM' : 'AM';
  hh %= 12;
  if (!hh) hh = 12;
  return `${M + 1}/${D}/${YYYY}, ${hh}:${mm}:${ss} ${a}`;
}
...
function getCountWeekendsInMonth(month, year) {
  ...
  let count = 0;
  for (let i = 1; i <= lengthOfMonth; i += 1) {
...
function getWeekNumberByDate(date) {
  ...
  let count = Math.ceil((diff + start.getDay() * 86400000) / (86400000 * 7));
  if (start.getDay() === 0 && date.getDay() !== 0) {
    count += 1;
  }
  return count;
}
...
function getNextFridayThe13th(date) {
  ...
  for (let i = startMonth; i < 12; i += 1) {
    currentMonth = new Date(currentYear, i, 13);
